...
const fileItems = tv({
    slots: {
        ...
        icon: "rounded-full border-3 border-violet-100 bg-violet-200 p-2 text-violet-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-500",
        deleteButton: "",
    },
    ...
    variants: {
        state: {
            ...
            error: {
                ...
                deleteButton:
                    "hover:text-error-900 text-error-700 dark:text-error-400 dark:hover:text-error-300",
            },
        },
    },
    defaultVariants: {
...
export function FileItem({ name, size, state }: FileItemProps) {
    const { container, icon, deleteButton } = fileItems({ state });
        ...
        <div className={container()}>
            ...
            {state === "error" ? (
                <div className="flex flex-1 flex-col items-start gap-1">
                    ...
                    <button
                        type="button"
                        className="text-sm font-semibold text-error-700 hover:text-error-900 cursor-pointer dark:text-error-400 dark:hover:text-error-300  "
                    >
                        Try again
                    </button>
                </div>
